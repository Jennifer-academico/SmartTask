...
| Encapsulamiento | Todos los atributos de Tarea y sus clases hijas son privados, con getters y setters que validan antes de modificar el estado |
...
| Herencia | TareaNormal extiende de Tarea y TareaUrgente extiende de Tarea |
| Polimorfismo | GestorTareas maneja una única lista de tareas sin importar el tipo concreto; al llamar obtenerDetalle sobre cualquier tarea, Java ejecuta automáticamente la versión correcta según el tipo real del objeto |
...
En una versión intermedia, pedía la fecha de vencimiento como un texto libre sin exigir ningún formato, lo que dejaba que la persona escribiera cualquier cosa (mañana, 15 agosto, un formato distinto cada vez), generando datos inconsistentes que además no me servían para calcular nada.
